!> \file solvers_jakub.f90
!! jednoduche resice

module solvers_jakub
    public :: SDS
    public :: CGS
    public :: Axpy
    public :: AAxpy
...
contains
    ...
    subroutine SDS(U,x,b)
        ...
        do
            ...
            w1 = 0
            call Axpy(U,y,w1)  ! w1 = A*(b-A*x) == residuum z definice problemu
            ...
            if (pcnt == stepcnt) then
                ...
                " rcnt=",rcnt," integral = ",integral(U,xx)
                print *,  rar1, sqrt(rn1)
                pcnt = 1
            else
                pcnt = pcnt + 1
    ...
    subroutine Axpy(U,x,y)
        ...
        !print *,"axpy"
        do i=1,nt
            alfa = U%LocList(i)%alfa
            beta = U%LocList(i)%beta
            la   = U%LocList(i)%A
            mapa = U%LocList(i)%map(:,1)
            ly = x(mapa,:)
            do j=1,3
                if(mapa(j)==0)  then
                    ly(j,1) = 0
                    ...
                    if (U%Pointlist(mapa(j))%onboundary) then
        ...
        y(U%Bindex,:)=0
        do i=1,ubound(y,1)
            if ( .not. U%Pointlist(i)%used) y(i,:) = 0
            if ( U%Pointlist(i)%onboundary) y(i,:) = 0
        end do
    end subroutine Axpy


    subroutine AAxpy(U,x,y)
        use pmatypy
        use geometry
        use local
        use global
    ...
    subroutine getRhs(U,x)
        ...
        do i=1,nt
            x(U%LocList(i)%map(:,1),:) =&
            x(U%LocList(i)%map(:,1),:) + U%LocList(i)%b
        end do
        x(0,:) = 0
        do i = 1, ubound(x,1)
            if ( .not. U%PointList(i)%used ) then
                x(i,:)=0
            end if
            if (U%PointList(i)%onboundary) then
                x(i,:) = 0
    ...
    function integral(U,x) result (y)
        use pmatypy
        use geometry
        use local
        use global
        ...
        y = 0
        nt = ubound(U%LocList,1)
        nb = ubound(U%BIndex,1)
        !napsat do x spravne hodnoty na hranici
        do i=1,nb
            X(U%BIndex(I),:) = U%PointList(U%Bindex(i))%value
        end do
        do i=1,nt
            xl = x(U%LocList(i)%map(:,2),1)
            x2 = x(U%LocList(i)%map(:,2),2)
            wrk = DOT_PRODUCT(xl,matmul(U%LocList(i)%A,xl))*U%LocList(i)%beta
            y(1) = y(1) + wrk
            wrk = DOT_PRODUCT(x2,matmul(U%LocList(i)%A,x2))*U%LocList(i)%alfa
            y(2) = y(2) + wrk
           !print *, i, wrk
        end do
        !zase je vynulovat
    ...
    subroutine CGS(U,x,b)
        ...
        do
            ...
            if (resnorm == 0) return
            w1 = 0
            call Axpy(U,w2,w1)  ! w1 = A*(b-A*x) == residuum
            if (cnt == 1) then
                sm = w1
            else
            ...
            if ( (pcnt == stepcnt) .or. (rcnt > 0) ) then
                ...
                " rcnt=",rcnt," integral = ",integral(U,xx)," delka smeru=", sqrt(rn), "nejmensi:",rmin
                pcnt = 1
            else
                pcnt = pcnt + 1
            end if
